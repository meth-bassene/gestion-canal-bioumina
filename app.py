...
/* LABELS — noir par defaut, blanc sur fonds noirs */
...
/* Blanc dans prix-box */
.prix-box, .prix-box * { color:#ffffff !important; }
.prix-box .plabel { color:#cccccc !important; }
/* Blanc dans total-box */
.total-box, .total-box * { color:#ffffff !important; }
/* Blanc dans token-box */
.token-box, .token-box * { color:#ffffff !important; }
/* Blanc dans card header mobile */
...
def init_db():
    ...
    if not c.fetchone():
        ...
        c.execute("INSERT INTO users (username,telephone,password,role,nom_complet,date_creation) VALUES (?,?,?,?,?,?)",
                  ("admin","000000000",h.decode(),"admin","Compte Principal",datetime.now().strftime("%Y-%m-%d")))
    else:
        h = bcrypt.hashpw("Madinatou1432".encode(), bcrypt.gensalt())
...
def get_ventes_jour():
    ...
    df = pd.read_sql_query(f"""
        SELECT u.nom_complet as vendeur, COUNT(d.id) as ventes_jour,
               COALESCE(SUM(d.prix_total),0) as ca_jour
        FROM users u LEFT JOIN decodeurs d ON d.affecte_a=u.username
            AND d.statut='vendu' AND d.date_activation LIKE '{today}%'
        GROUP BY u.username, u.nom_complet ORDER BY ventes_jour DESC
    """, conn)
    conn.close()
    return df
...
def get_vendeurs():
    ...
    rows = c.fetchall()
    conn.close()
    return rows if rows else [("admin","Compte Principal")]
...
for k,v in [('connecte',False),('mode_token',False),('confirmer_vente',False),('vente_count',0)]:
    ...
    # ══ VENTE ════════════════════════════════════════════════
        ...
        if df_dispo.empty:
            ...
            with c1:
                ...
                client_nom = st.text_input("Nom du client", key=f"vente_client_nom_{st.session_state.vente_count}")
                client_tel = st.text_input("Numero de telephone du client", key=f"vente_client_tel_{st.session_state.vente_count}")
            ...
            if not st.session_state.confirmer_vente:
                ...
                with col_oui:
                    if st.button("Oui confirmer", use_container_width=True):
                        ...
                        # Vider les champs en changeant les clés
                        st.session_state.vente_count += 1
